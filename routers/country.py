...
from schemas import CountryModel, AdminModel
from update_schemas import UpdateCountryModel
from oauth2 import get_current_user
from typing import Optional, List

router = APIRouter(prefix="/countr", tags=["Countries"])


@router.post(
    "y",
    response_description="Add new country",
    response_model=CountryModel,
    status_code=status.HTTP_201_CREATED,
)
async def create_country(
    country: CountryModel = Body(...),
...
@router.get(
    "ies",
    response_description="List all countries",
...
@router.get(
    "y",
    response_description="Get a single country",
    response_model=CountryModel,
    status_code=status.HTTP_200_OK,
)
async def show_country(
    ...
    """
    variables = locals()
    options = {"identifier": "_id", "country_name": "name", "capital": "capital"}
...
@router.put(
    "y",
    response_description="Update a country",
    response_model=UpdateCountryModel,
    status_code=status.HTTP_200_OK,
)
    ...
    for key in variables.keys():
        if variables[key] is not None:
            return await update_object(
                {options[key]: variables[key]}, country, "countries"
            )
    raise HTTPException(status_code=404, detail="Set some parameters")


@router.delete(
    "y", response_description="Delete a country", status_code=status.HTTP_204_NO_CONTENT
)
