...
@router.post(
    "",
    response_description="Add new admin",
    response_model=AdminModel,
    status_code=status.HTTP_201_CREATED,
)
async def create_admin(
    admin: AdminModel = Body(...), current_user: AdminModel = Depends(get_current_user)
):
    """
...
@router.get(
    "s",
    response_description="List all admins",
    response_model=List[AdminModel],
...
@router.get(
    "",
    response_description="Get a single admin",
    response_model=AdminModel,
    status_code=status.HTTP_200_OK,
)
async def show_admin(
    identifier: Optional[str] = None, admin_name: Optional[str] = None
):
    """
    Get admin by name:
...
@router.put(
    "",
    response_description="Update an admin",
    response_model=UpdateAdminModel,
    status_code=status.HTTP_200_OK,
)
...
@router.delete(
    "", response_description="Delete an admin", status_code=status.HTTP_204_NO_CONTENT
)
