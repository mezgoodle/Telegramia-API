...
from schemas import ItemModel, AdminModel
from update_schemas import UpdateItemModel
...
async def create_item(
    item: ItemModel = Body(...), current_user: AdminModel = Depends(get_current_user)
):
    """
    Create an item:
    ...
    for key in variables.keys():
        if variables[key] is not None:
            return await get_object(
                {options[key]: variables[key], "city": city_name}, "items"
            )
    ...
    for key in variables.keys():
        if variables[key] is not None:
            return await update_object(
                {options[key]: variables[key], "city": city_name}, item, "items"
            )
    raise HTTPException(status_code=404, detail="Set some parameters")


@router.delete(
    "", response_description="Delete an item", status_code=status.HTTP_204_NO_CONTENT
)
