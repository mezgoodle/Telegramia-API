...
@router.post('', response_description='Add new dungeon', response_model=DungeonModel,
             status_code=status.HTTP_201_CREATED)
async def create_dungeon(dungeon: DungeonModel = Body(...), current_user: AdminModel = Depends(get_current_user)):
    """
    Create a dungeon:
...
@router.get('s', response_description='List all dungeons', response_model=List[DungeonModel],
...
@router.get('', response_description='Get a single dungeon', response_model=DungeonModel,
            status_code=status.HTTP_200_OK)
async def show_dungeon(identifier: Optional[str] = None, dungeon_name: Optional[str] = None):
    """
...
@router.put('', response_description='Update a dungeon', response_model=UpdateDungeonModel,
            status_code=status.HTTP_200_OK)
async def update_dungeon(identifier: Optional[str] = None, dungeon_name: Optional[str] = None,
    ...
    for key in variables.keys():
        if variables[key] is not None:
            return await update_object({options[key]: variables[key]}, dungeon, 'dungeons')
    raise HTTPException(status_code=404, detail='Set some parameters')


@router.delete('', response_description='Delete a dungeon',
               status_code=status.HTTP_204_NO_CONTENT)
async def delete_dungeon(identifier: Optional[str] = None, dungeon_name: Optional[str] = None,
                         current_user: AdminModel = Depends(get_current_user)):
    """
    Delete a dungeon:
