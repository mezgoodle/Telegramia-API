...
router = APIRouter(prefix="/cit", tags=["Cities"])
...
async def create_city(
    city: CityModel = Body(...), current_user: AdminModel = Depends(get_current_user)
):
    """
    Create a city:
...
@router.get(
    "y",
    response_description="Get a single city",
    response_model=CityModel,
    status_code=status.HTTP_200_OK,
)
...
@router.put(
    "y",
    response_description="Update a city",
    response_model=UpdateCityModel,
    status_code=status.HTTP_200_OK,
)
...
@router.delete(
    "y", response_description="Delete a city", status_code=status.HTTP_204_NO_CONTENT
)
